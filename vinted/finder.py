...
from .phone import (detect_model, is_accessory, find_defects, extract_storage, extract_battery,
                    CONDITION_FACTOR, estimate_value, estimate_profit, MODEL_ORDER)
...
class Run:
    ...
    def evaluate(self, item, drops):
        ...
        best_case = quote.price * CONDITION_FACTOR.get(get_condition(item), 1.08) * 1.03
        if price > best_case * (1 - c["MIN_DISCOUNT"]):
            self.new_seen[iid] = time.time()
            return self.reject("per brangu")
        if price < quote.price * c["HARD_MIN_PRICE_RATIO"]:
            self.new_seen[iid] = time.time()
            return self.reject("per pigu telefonui (priedas?)", f"{title[:40]} {price:.0f}€ (rinka {quote.price:.0f}€)")
        self.new_seen[iid] = time.time()
        ...
        if any(f == 0 for _, f in defects):
            return self.reject("iCloud / užblokuotas / dalims", f"{title[:40]} {[d for d, _ in defects]}")
