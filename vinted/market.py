...
from . import config
from .phone import detect_model, extract_storage, is_accessory, find_defects
...
class Market:
    ...
    def quote(self, model, storage, day=None):
        ...
        if c["USE_SOLD_PRICES"]:
            for by_storage in ([True, False] if storage else [False]):
                sold = values("sold", c["SOLD_HISTORY_DAYS"], by_storage, "sd")
                if len(sold) >= c["MIN_SOLD_SAMPLES"]:
                    return Quote(median(sold), len(sold), "parduoti", by_storage)
        for by_storage in ([True, False] if storage else [False]):
            asking = values("active", c["PRICE_HISTORY_DAYS"], by_storage, "l")
    ...
    def summary(self, day=None):
        ...
        for model, d in by_model.items():
            out.append((model, percentile(d["active"], c["MARKET_PERCENTILE"]) if d["active"] else None,
                        len(d["active"]), median(d["sold"]) if d["sold"] else None, len(d["sold"])))
        return out
